...
function cleanRequestTimestamps(timestamps, nowMs) {
...
async function readLimitState(url) {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Snapshot request-limit store returned HTTP ${response.status}: ${details}`
    );
  }
...
async function reserveSnapshotRequest(userId) {
  ...
  for (
    ...
    if (written) {
      const used =
        updatedState.requestTimestampsUtc.length;
  ...
  throw new Error(
    "Unable to reserve the snapshot request quota because the rate-limit record changed repeatedly."
  );
}
